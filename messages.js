...
async function sendDirectMessage() {
    ...
    try {
        ...
        // 🛠️ نسجّل معرف الحدث كـ "شفناه" فورًا بعد التوقيع، قبل النشر —
        // لو سجّلناه بعد publishToRelays، فيه احتمال (نادر بس حقيقي) إن
...
async function acceptDmCall() {
    ...
    try {
        ...
        if (pubkey) sendCallSignal(pubkey, 'reject'); // نبلّغ المتصل فورًا بدل ما يستنى المهلة كاملة
        endDmCall(true);
        return;
    }
    ...
    try { peer = await ensureDmPeer(); }
        ...
        if (pubkey) sendCallSignal(pubkey, 'reject');
        endDmCall(true);
        return;
    }
